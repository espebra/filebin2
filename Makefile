...
default:
	...
	go test -cover -v -mod=vendor -coverprofile=cover.out dbl/*.go | tee dbl.out
	cat dbl.out | go-junit-report > tests/dbl.xml
	go tool cover -html=cover.out -o artifacts/dbl.html
	go tool cover -func=cover.out

	go test -cover -v -mod=vendor -coverprofile=cover.out s3/*.go | tee s3.out
	cat s3.out | go-junit-report > tests/s3.xml
	go tool cover -html=cover.out -o artifacts/s3.html
	go tool cover -func=cover.out

	go test -cover -v -mod=vendor -coverprofile=cover.out *.go | tee http.out
	cat http.out | go-junit-report > tests/http.xml
	go tool cover -html=cover.out -o artifacts/http.html
	go tool cover -func=cover.out
