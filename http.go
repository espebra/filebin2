...
func (h *HTTP) Init() (err error) {
	...
	h.router.HandleFunc("/admin/approve/{bin:[A-Za-z0-9_-]+}", h.Auth(h.ApproveBin)).Methods("PUT")
	...
	h.router.HandleFunc("/{bin:[A-Za-z0-9_-]+}/{filename:.+}", h.UploadFile).Methods(http.MethodPost)
